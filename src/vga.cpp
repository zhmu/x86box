...
#define TRACE_INT(x...) \
	if (1) fprintf(stderr, "[vga-int] " x)
#define TRACE(x...) \
	if (1) fprintf(stderr, "[vga] " x)
...
uint8_t
VGA::ReadByte(Memory::addr_t addr)
{
	TRACE("read(8) @ 0x%04x\n", addr);
...
uint16_t
VGA::ReadWord(Memory::addr_t addr)
{
...
void
VGA::WriteByte(Memory::addr_t addr, uint8_t data)
{
...
void
VGA::WriteWord(Memory::addr_t addr, uint16_t data)
{
...
void
VGA::InvokeVector(uint8_t no, CPUx86& oCPU, CPUx86::State& oState)
{
...
#define GET_BH uint8_t bh = (oState.m_bx & 0xff00) >> 8;
	...
	switch(ah) {
		case 0x0f:
			...
			oState.m_ax =
			 m_memory.ReadByte(CPUx86::MakeAddr(0x40, 0x49)) |
			 m_memory.ReadByte(CPUx86::MakeAddr(0x40, 0x4a)) << 8;
			oState.m_bx = (oState.m_bx & 0xff) |
			 m_memory.ReadByte(CPUx86::MakeAddr(0x40, 0x62)) << 8;
			break;
		case 0x08: {
			GET_BH;
			TRACE_INT("ah=%02x: read character and attribute at cursor position, page=%u\n", ah, bh);
			oState.m_ax = 0x1e20;
			break;
		}
		case 0x11: {
			...
			switch(al) {
				case 0x30:
					...
					oState.m_es = 0; oState.m_bp = 0; /* XXX */
					oState.m_cx = 0; // XXX
