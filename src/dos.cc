...
#define TRACE_INT(x...) \
	fprintf(stderr, "[dos-int] " x)
#define TRACE_EXE(x...) \
	fprintf(stderr, "[dos-exe] " x)
...
DOS::LoadEXE(XFile& oFile)
{
	...
	/* Create a PSP */
	struct ProgramSegmentPrefix* oPSP = (struct ProgramSegmentPrefix*)m_Memory.GetPointer(CPUx86::MakeAddr(seg, 0), sizeof(*oPSP));
	CreatePSP(*oPSP);

	TRACE_EXE("exe_size=%u seg=0x%04x exe_seg=%04x bss_seg=%04x\n",
	 exe_size, seg, exe_seg, bss_seg);
	TRACE_EXE("cs:ip=%04x:%04x ss:sp=%04x:%04x\n",
	 header.eh_cs, header.eh_ip, header.eh_ss, header.eh_sp);

	/* Program data starts after the header; read it completely */
	TRACE_EXE("loading %u bytes from offset 0x%x -> %04x:0000 (%x)\n",
	 exe_size, header.eh_header_paragraphs * 16, exe_seg,
	 CPUx86::MakeAddr(exe_seg, 0));
...
void DOS::CreatePSP(struct ProgramSegmentPrefix& oPSP)
{
	...
	oPSP.psp_exit[0] = 0xcd; oPSP.psp_exit[1] = 0x20; /* INT 20 */
	oPSP.psp_fn_disp[0] = 0xcd; oPSP.psp_fn_disp[1] = 0x21; /* INT 21 */
...
#define SET_ERROR(n) \
	...
	switch(ah) {
		...
		case 0x3c: /* create/truncate file */ {
			...
			TRACE_INT("ah=%02x: create/truncate file, cx=%04x ds:dx=%04x:%04x '%s'\n", ah, CX, DS, DX, sFilename);
