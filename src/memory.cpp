...
void
Memory::WriteByte(addr_t addr, uint8_t data)
{
...
void
Memory::WriteWord(addr_t addr, uint16_t data)
{
...
void
Memory::AddPeripheral(addr_t base, uint16_t length, XMemoryMapped& oPeripheral)
{
	m_peripheral.push_back(MemoryMapped(base, length, oPeripheral));
}

char*
Memory::GetASCIIZString(addr_t addr)
{
	char* ptr = m_String;
...
void*
Memory::GetPointer(addr_t addr, uint16_t length)
{
	for (TPeripheralList::iterator it = m_peripheral.begin(); it != m_peripheral.end(); it++)
		if (it->Matches(addr))
			return NULL;
