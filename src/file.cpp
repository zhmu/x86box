...
bool File::Open(const char* fname, int flags)
{
...
File::off_t File::GetLength() const
{
    FILE* f = (FILE*)m_File;
    if (f == NULL)
        return 0;
...
File::ssize_t File::Read(void* buffer, ssize_t size)
{
    FILE* f = (FILE*)m_File;
    if (f == NULL)
        return -1;
...
File::ssize_t File::Write(const void* buffer, ssize_t size)
{
    FILE* f = (FILE*)m_File;
    if (f == NULL)
        return -1;
